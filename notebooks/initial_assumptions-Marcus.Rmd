...
model_base_controls <- lm(us_views_millions ~ Neutral_percent + directed_by + written_by + air_date, data=episode_by_emotions)
model_all_emotions <- lm(us_views_millions ~ Joyful_percent + Mad_percent + Peaceful_percent + Powerful_percent + Sad_percent + directed_by + written_by + air_date, data=episode_by_emotions)
...
The variables here are not linear combinations of each other; there is no perfect colinearity as seen in the lack of perfect correlation in the matrix below.

```{r}
cor(episode_by_emotions[c(1:17,23)])
...
residualPlots(model_base_controls)
residualPlots(model_all_emotions)
...
print("Base model")
plot(model_base)
print("Base model with controls")
plot(model_base_controls)
print("Model with all emotions")
plot(model_all_emotions)
```
